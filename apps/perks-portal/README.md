...
Search, a labeled sort menu, and rounded type-filter buttons share one form. Category sorting follows the filter order and alphabetizes partners within each type; Alphabetical (A–Z) orders all partners by name. The sort choice combines with search and filters, survives refresh through the URL, and is retained when clearing filters. All controls work without JavaScript; when JavaScript is available, results update immediately. On small screens, search and sort each get a full row and the filter buttons wrap, retaining at least 44px tap targets. Desktop entries use separate Partner, Type, and Perk columns.
