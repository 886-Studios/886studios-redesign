...
export function renderDirectory(catalog, { query = '', category = '', basePath = '' } = {}) {
  ...
  <div class="directory-footer"><p class="directory-note">For ikigai Launchpad and Launch Station teams only. Please keep partner links and codes within your company. Offers are subject to partner eligibility and approval.</p><p class="perk-request">Have any products &amp; services you use but that's not on this list? Contact Carter &amp; Patryk and we'll try to get them for you!</p></div></main>`, { basePath });
}
